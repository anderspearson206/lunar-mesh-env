"""
Experiment C — PPO training with epsilon-greedy communication.

Identical to train_ppo_mlp_gat.py except:
  - Env class is LunarRoverMeshEpsCommEnv
  - EpsCommDecayCallback schedules eps_comm 1.0 → 0.0 over the first half
    of training, forcing epidemic routing early so the value function trains
    on high-delivery episodes before the RL comm head takes over.

Usage:
    python examples/train_ppo_eps_comm.py
    python examples/train_ppo_eps_comm.py --maps /path/to/radio_maps_hm_18.npy
"""
...
TOTAL_ITERATIONS = 50
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "lunar_mesh_ppo_eps_comm"
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshEpsCommEnv(
    ...
    def on_train_result(self, *, algorithm=None, result, **_):
        ...
        def _set_eps(env):
            if hasattr(env, "eps_comm"):
                env.eps_comm = eps
            # ParallelPettingZooEnv wraps the raw env
...
register_env("lunar_mesh_eps_comm_v1", env_creator)
