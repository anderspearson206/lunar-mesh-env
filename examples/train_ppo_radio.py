"""
PPO training with the MLP-GAT + radio-map encoding policy.

Three radio encoding strategies are selectable via --radio:
  rssi  – 4 continuous RSSI scalars appended to MLP scalar branch
  crop  – 64×64 local radio-map crop processed by a small CNN
  full  – full 256×256 radio map processed by a deep CNN (memory-intensive)

Usage:
    python examples/train_ppo_radio.py --radio rssi
    python examples/train_ppo_radio.py --radio crop  --maps /path/to/radio_maps_hm_18.npy
    python examples/train_ppo_radio.py --radio full  --iterations 100
"""
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshRadioEnv(
        ...
        radio_obs=config.get("radio_obs", "rssi"),
        seed=19,
    )
...
# ---------------------------------------------------------------------------
def build_config(obs_space, act_space, maps_path: str, radio_mode: str, run_name: str):
    env_name = f"lunar_mesh_radio_{radio_mode}_v1"
    return (
        PPOConfig()
        .environment(
            env_name,
        ...
        .training(
            model={
                ...
                "custom_model_config":       {"radio_mode": radio_mode},
...
if __name__ == "__main__":
    ...
    parser.add_argument("--radio",      choices=["rssi", "crop", "full"], default="rssi",
                        help="Radio encoding strategy (default: rssi)")
    parser.add_argument("--maps",       default=MAPS_PATH,
                        help="Path to precomputed radio maps .npy")
    parser.add_argument("--name",       default=None,
                        help="Run name for W&B and Ray (default: lunar_mesh_ppo_radio_<mode>)")
    ...
    run_name = args.name or f"lunar_mesh_ppo_radio_{args.radio}"
    ...
    config      = build_config(obs_space, act_space, args.maps, args.radio, run_name)
