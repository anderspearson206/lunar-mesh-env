...
All three fixes combined: A* removes movement gradient, eps_comm seeds
the value function with epidemic-level delivery during early training,
touched reward gives proper relay credit at delivery time.

Movement is provided by A* (heuristic_move_action); only the GAT comm
head is trained. This isolates comm gradient from the competing dense
movement reward.

When a packet is uniquely delivered to the BS, every agent that touched
it (originator + relay hops) receives REWARD_RELAY. This fixes the
credit assignment gap: relay agents now get gradient tied to actual
delivery outcomes.

Usage:
    python examples/train_ppo_astar_eps_comm.py
    python examples/train_ppo_astar_eps_comm.py --maps /path/to/radio_maps_hm_18.npy
"""
...
TOTAL_ITERATIONS = 50
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "lunar_mesh_ppo_astar_eps_comm"
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshAStarEpsCommEnv(
...
class EpsCommDecayCallback(DefaultCallbacks):
    """Linear decay of eps_comm from 1.0 → 0.0 over the first half of training."""

    def on_train_result(self, *, algorithm=None, result, **_):
        it  = result.get("training_iteration", 0)
        eps = max(0.0, 1.0 - it / (TOTAL_ITERATIONS / 2))

        def _set_eps(env):
            if hasattr(env, "eps_comm"):
                env.eps_comm = eps
            if hasattr(env, "env") and hasattr(env.env, "eps_comm"):
                env.env.eps_comm = eps

        for attr in ("workers", "env_runner_group"):
            obj = getattr(algorithm, attr, None)
            if obj is None or callable(obj):
                continue
            if hasattr(obj, "foreach_worker"):
                obj.foreach_worker(lambda w: w.foreach_env(_set_eps))
            elif hasattr(obj, "foreach_env"):
                obj.foreach_env(_set_eps)
            break
        ...
        print(f"[iter {it:>4}]  eps_comm={eps:.3f}  episode_reward_mean={rew}  episodes={n_eps}")
        if wandb.run is not None and rew is not None and not np.isnan(float(rew)):
            wandb.log({"episode_reward_mean": rew,
                       "eps_comm":            eps,
                       "episodes_this_iter":  n_eps or 0}, step=it)
...
register_env("lunar_mesh_astar_eps_comm_v1", env_creator)
...
def build_config(obs_space, act_space, maps_path: str):
    return (
        ...
        .callbacks(EpsCommDecayCallback)
        .multi_agent(
