"""
PPO training for LunarRoverMeshEnv using precomputed radio maps.

Radio maps are served from a memory-mapped numpy array produced by
examples/precompute_radio_maps.py, replacing real-time NN inference.
Steps run in microseconds instead of ~1 s, making training ~1000× faster.

Usage:
    python examples/train_ppo_lookup.py
    python examples/train_ppo_lookup.py --maps /path/to/radio_maps_hm_18.npy

Adjust DATA_ROOT and MAPS_PATH to point to your data.
"""
...
MAPS_PATH  = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "radio_maps_hm_18.npy")   # output of precompute_radio_maps.py
