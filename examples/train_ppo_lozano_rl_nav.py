"""
PPO training with Lozano-style single-edge comm + RL-controlled navigation.

Navigation: RL (9-direction action[0], trained jointly with comm)
Comm action: single edge selection over {hold, peer0, peer1, BS}
             rate-fill to selected target via send_packets_to_target

Compare against train_ppo_lozano.py (A* nav) to isolate the benefit of
learning navigation vs. using the heuristic.

Usage:
    python examples/train_ppo_lozano_rl_nav.py
    python examples/train_ppo_lozano_rl_nav.py --name my_run --iterations 200
"""
...
# ---------------------------------------------------------------------------
# Training hyper-parameters  (same as train_ppo_lozano.py)
# ---------------------------------------------------------------------------
NUM_AGENTS       = 3
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshLozanoRLNavEnv(
...
class NavCurriculumCallback(DefaultCallbacks):
    """Decay eps_nav 1.0 → 0.0 over first half of training (nav-first curriculum)."""

    def on_train_result(self, *, algorithm=None, result, **_):
        it  = result.get("training_iteration", 0)
        eps_nav = max(0.0, 1.0 - it / (TOTAL_ITERATIONS / 2))
        ...
        print(f"[iter {it:>4}]  eps_nav={eps_nav:.3f}  episode_reward_mean={rew}  episodes={n_eps}")
        if wandb.run is not None and rew is not None and not np.isnan(float(rew)):
            log = {"episode_reward_mean": rew, "eps_nav": eps_nav, "episodes_this_iter": n_eps or 0}
...
register_env("lunar_mesh_lozano_rl_nav_v1", env_creator)
...
def build_config(obs_space, act_space, maps_path: str):
    return (
        ...
        .callbacks(NavCurriculumCallback)
        .multi_agent(
