"""
PPO training: Lozano RL-nav + radio-map scalar features.

Same as train_ppo_lozano_rl_nav.py but uses:
  LunarRoverMeshLozanoRLNavRadioEnv  — adds radio_at_goal + bs_grad_at_agent to obs
  TorchMLPGATLozanoRadioModel        — MLP branch extended to consume those features

Usage:
    python examples/train_ppo_lozano_rl_nav_radio.py
    python examples/train_ppo_lozano_rl_nav_radio.py --name my_run --iterations 200
"""
...
TOTAL_ITERATIONS = 50
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "lozano_rl_nav_radio"
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshLozanoRLNavRadioEnv(
...
register_env("lunar_mesh_lozano_rl_nav_radio_v1", env_creator)
...
def build_config(obs_space, act_space, maps_path: str):
    return (
        PPOConfig()
        .environment(
            "lunar_mesh_lozano_rl_nav_radio_v1",
            env_config={
