"""
PPO training: Lozano RL-nav + CNN-encoded BS radio map.

Same as train_ppo_lozano_rl_nav.py but uses:
  LunarRoverMeshLozanoRLNavCNNEnv  — adds bs_radio_obs (1×256×256) to obs
  TorchMLPGATLozanoCNNModel        — CNN encoder compresses map to 64-dim embedding

Usage:
    python examples/train_ppo_lozano_rl_nav_cnn.py
    python examples/train_ppo_lozano_rl_nav_cnn.py --name my_run --iterations 200
"""
...
TOTAL_ITERATIONS = 50
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "lozano_rl_nav_cnn"
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshLozanoRLNavCNNEnv(
...
register_env("lunar_mesh_lozano_rl_nav_cnn_v1", env_creator)
...
def build_config(obs_space, act_space, maps_path: str):
    return (
        PPOConfig()
        .environment(
            "lunar_mesh_lozano_rl_nav_cnn_v1",
            env_config={
