"""
PPO training: Lozano RL-nav + local BS radio map crop (position-aware CNN).

Same as train_ppo_lozano_rl_nav.py but uses:
  LunarRoverMeshLozanoRLNavCropEnv  — adds bs_crop_obs (1×64×64) centred on agent
  TorchMLPGATLozanoCropModel        — 2-layer CNN compresses crop to 64-dim embedding

Usage:
    python examples/train_ppo_lozano_rl_nav_crop.py
    python examples/train_ppo_lozano_rl_nav_crop.py --name my_run --iterations 200
"""
...
TOTAL_ITERATIONS = 50
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "lozano_rl_nav_crop"
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshLozanoRLNavCropEnv(
...
class NavCurriculumCallback(DefaultCallbacks):
    """Decay eps_nav 1.0 → 0.0 over first half of training."""
...
register_env("lunar_mesh_lozano_rl_nav_crop_v1", env_creator)
...
def build_config(obs_space, act_space, maps_path: str):
    return (
        PPOConfig()
        .environment(
            "lunar_mesh_lozano_rl_nav_crop_v1",
            env_config={
