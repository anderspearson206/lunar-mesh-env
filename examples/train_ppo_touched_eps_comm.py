"""
Experiment E — PPO training with touched relay credit + epsilon-greedy comm.

Combines Exp D (touched credit assignment) with Exp C (epsilon-greedy
comm decay): eps_comm seeds exploration of relay behavior during early
training; the touched reward gives proper gradient when those relays
contribute to actual deliveries.

eps_comm is decayed 1.0 → 0.0 linearly over the first half of training
by EpsCommDecayCallback.

Usage:
    python examples/train_ppo_touched_eps_comm.py
    python examples/train_ppo_touched_eps_comm.py --maps /path/to/radio_maps_hm_18.npy
"""
...
TOTAL_ITERATIONS = 50
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "lunar_mesh_ppo_touched_eps_comm"
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshTouchedEpsCommEnv(
...
register_env("lunar_mesh_touched_eps_comm_v1", env_creator)
...
def build_config(obs_space, act_space, maps_path: str):
    return (
        PPOConfig()
        .environment(
            "lunar_mesh_touched_eps_comm_v1",
            env_config={
