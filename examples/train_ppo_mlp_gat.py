"""
PPO training with the hybrid MLP-GAT policy for LunarRoverMeshEnv.

Movement decisions (9 actions) use an MLP over scalar features.
Communication decisions (8 actions = 4 targets × 2) use a GAT over
the local agent-BS graph.

Usage:
    python examples/train_ppo_mlp_gat.py
    python examples/train_ppo_mlp_gat.py --maps /path/to/radio_maps_hm_18.npy
"""
...
TOTAL_ITERATIONS = 50
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "lunar_mesh_ppo_mlp_gat"
...
register_env("lunar_mesh_mlp_gat_v1", env_creator)
