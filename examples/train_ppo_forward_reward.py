"""
Experiment B — PPO training with dense forwarding reward.

Identical to train_ppo_mlp_gat.py except the env class is
LunarRoverMeshForwardRewardEnv, which gives a small reward each time
an agent forwards a new packet to a peer rover.

Usage:
    python examples/train_ppo_forward_reward.py
    python examples/train_ppo_forward_reward.py --maps /path/to/radio_maps_hm_18.npy
"""
...
TOTAL_ITERATIONS = 50
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "lunar_mesh_ppo_forward_reward"
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshForwardRewardEnv(
...
register_env("lunar_mesh_forward_reward_v1", env_creator)
