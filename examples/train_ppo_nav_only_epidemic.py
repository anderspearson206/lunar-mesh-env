"""
PPO training: nav-only RL with epidemic communication.

The policy controls only 9-way movement (action_space = Discrete(9)).
Epidemic routing handles all packet delivery automatically each step.
This isolates whether the policy learns to navigate through better signal areas.

Compare against:
  train_ppo_lozano_rl_nav.py  — RL nav + RL comm (Lozano single-edge)
  diagnose_packet_fate.py     — A* nav + epidemic (delivery ceiling benchmark)

Usage:
    python examples/train_ppo_nav_only_epidemic.py
    python examples/train_ppo_nav_only_epidemic.py --name my_run --iterations 200
    python examples/train_ppo_nav_only_epidemic.py --ttl 200
"""
...
CHECKPOINT_FREQ  = 50
EXPERIMENT_NAME  = "nav_only_epidemic"
...
def env_creator(config):
    ...
    raw_env = LunarRoverMeshNavOnlyEpidemicEnv(
...
register_env("lunar_mesh_nav_only_epidemic_v1", env_creator)
...
def build_config(obs_space, act_space, maps_path: str, extra_env_cfg: dict | None = None):
    ...
    return (
        PPOConfig()
        .environment(
            "lunar_mesh_nav_only_epidemic_v1",
            env_config=env_cfg,
        )
        .framework("torch")
        .api_stack(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ttl", type=int, default=None,
                        help="Packet TTL in steps (default: env default, 5000 in rate mode)")
