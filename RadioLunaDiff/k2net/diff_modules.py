...
class K2_UNet(nn.Module):
    def __init__(self, inputs=3, k2_bin=False):
        super().__init__()
        self.inputs = inputs
        self.k2_bin = k2_bin
        ...
        first_unet_up_ch_in = 20 + 6 + inputs
        final_conv_ch_in = 20 + inputs
        ...
        self.conv_up00 = convrelu(first_unet_up_ch_in, 20, 5, 2, 1)

        if self.k2_bin:
            self.conv_up000 = nn.Conv2d(final_conv_ch_in, 1, kernel_size=5, padding=2)
        else:
            self.conv_up000 = convrelu(final_conv_ch_in, 1, 5, 2, 1)
