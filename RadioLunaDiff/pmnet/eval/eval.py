...
def NMSE(pred, target): 
  loss = nn.MSELoss()(pred, target)/nn.MSELoss()(target, 0*target)
  return loss
...
#   loss = pytorch_msssim.SSIM(data_range=1, channel=1)(pred, target)
#   return loss
...
def SSIM(pred, target):
  ...
  loss1_list = []
  for i in range(bs):
    loss1, _ = ssim(pred.cpu().numpy()[i][0], target.cpu().numpy()[i][0], full=True, data_range=1)
    loss1_list.append(loss1)
  loss = np.mean(loss1_list)
...
def PSNR(pred, target):
  ...
  loss1_list = []
  for i in range(bs):
    loss1 = psnr(pred.cpu().numpy()[i][0], target.cpu().numpy()[i][0], data_range=1)
    loss1_list.append(loss1)
  loss = np.mean(loss1_list)
  return torch.tensor(loss)
'''
